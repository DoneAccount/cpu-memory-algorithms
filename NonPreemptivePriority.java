...
public class NonPreemptivePriority {
    ...
    public static void schedule(List<Process> processes) {
        ...
        while (completed != n) {
            ...
            for (int i = 0; i < n; i++) {
                if (!isCompleted[i] && processes.get(i).getArrivalTime() <= currentTime && processes.get(i).getPriority() < minPriority) {
    ...
    public static void printTable(List<Process> processes) {
        System.out.printf("%-8s %-8s %-8s %-10s %-8s %-8s %-8s %-12s%n", "Process", "Arrival", "Priority", "CPU Cycle", "End Time", "Run Time", "TAT", "Waiting Time");
        ...
        for (Process p : ordered) {
            System.out.printf("%-8s %-8d %-8d %-10d %-8d %-8d %-8.2f %-12.2f%n", p.getName(), p.getArrivalTime(), p.getPriority(), p.getBurstTime(), p.getCompletionTime(), p.getTotalRunTime(), (double)p.getTurnaroundTime(), (double)p.getWaitingTime());
        }
    }
